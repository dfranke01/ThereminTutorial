...
songData* RTTL2SongData( char* RTTL, int *size);
...
void setup()
{
  ...
  // convert all RTTL songs to *Song type and populate songs array
  // get size of RTTLSongs[]
  int numRTTLSongs = sizeof(RTTLSongs) / sizeof(RTTLSongs[0]);
  for( int i = 0; i < numRTTLSongs; i++)
    {
      int newSize = 0;
      songData* tempSongData = RTTL2SongData(RTTLSongs[i], &newSize);
      // get name from RTTLSong
      char* p = RTTLSongs[i];
      int nameLength = 0;
      while(*p != ':'){p++; nameLength++;} 
      char newName[nameLength*sizeof(char)];
      strncpy( newName, RTTLSongs[i], (nameLength)*sizeof(char));
      Song *newSong = new Song::Song(newName, tempSongData, newSize);
      newSongs[numSongs] = newSong;
      numSongs++;  
      Serial.print("name = ");
      for(int j=0; j<nameLength; j++)
         Serial.print(newName[j]);
      ...
      char* p = RTTLSongs[i];
      int nameLength = 0;
      while(*p != ':'){p++; nameLength++;Serial.println(*p);} 
      
      char *newName = (char*)malloc((nameLength-1)*sizeof(char));
      memcpy( newName, RTTLSongs[i], (nameLength-1)*sizeof(char));
      Song *newSong = new Song::Song(newName, tempSongData, newSize);
      newSongs[numSongs] = newSong;
      numSongs++;  
      Serial.print("name = ");Serial.print(*newName);Serial.print(" size = ");Serial.println(newSize);
 */
    }  
  ...
  //newSong = RTTL2SongData( song, &newSongLength );
  //newSong = SWOTR;
  //newSongLength = sizeof(SWOTR) / sizeof(SWOTR[0]); // number of notes to play for a particular song
  //Serial.print("newSongLength = ");Serial.println(newSongLength);
  //for( int i = 0; i < newSongLength; i++)
  //{
      //Serial.print("note, duration, vol");Serial.print(newSong[i].note);Serial.print(" ");Serial.print(newSong[i].duration);Serial.print(" ");Serial.println(newSong[i].volume);
  //}
}
...
void loop()
{
  ...
  if( testNote != prevTestNote )
    {
     Serial.print("testNote = ");Serial.println(testNote);
     prevTestNote = testNote;
    }
}
...
songData* RTTL2SongData( char* RTTL, int *size)
{
  songData *newSong; // new song to populate
  char* p = RTTL;
  int num=0;
  int default_dur = 4;
  int default_oct = 4;
  ...
  newSongLength++;  // num of notes = num of commas + 1
  *size = newSongLength; // size to pass back
  newSong = (songData*)calloc(newSongLength, sizeof(songData));
  ...
  while(*p)
  {
    ...
    //Serial.print("num "); Serial.println(num, 10);
    ...
    // new duration = whole_note / (20*duration)
    newSong[iNote].duration = (int)duration/20; // need to allocate for newSong
    //Serial.print("duration ");Serial.print(duration);Serial.print(" duration ");Serial.println(duration/20);
    ...
    //newSong[iNote].note = notes[(scale - 4) * 12 + note];
    newSong[iNote].note = ((scale + 1) * 12) + (note - 1); // our scale starts at -1, and our indices star at zero, so use scale+1, subtract 1 from result
    newSong[iNote].volume = 10;
    iNote++;
    //Serial.print("note ");Serial.println(notes[(scale - 4) * 12 + note], 10);
  }
 }
